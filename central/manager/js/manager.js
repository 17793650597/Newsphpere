var ModuleConfig = {
		...
		loadModuleConfigs: function(){
			var menu = $('#nv_left').find('.menu.navi').empty();
			CentralProxy.getModulesInf({}, function(resp){
				for (var i in resp.items) {
					ModuleConfig.renderNavButton(resp.items[i], menu);
				}
		...
		renderContent: function(module) {
			var body = $('.config-field').empty();
			$('#nv_content').data('data',module);
			var pane = $('<div class="panel panel-default"><div class="panel-heading">固定属性</div><div class="panel-body"></div></div>').appendTo(body);
			var row = '<div class="form-group"><label class="col-sm-2 control-label">主键</label>\
				<div class="col-sm-10"><input type="text" class="form-control" placeholder="主键">\
				    </div></div>';
			$(row).appendTo(pane.find('.panel-body'));
			
			var pane2 = $('<div class="panel panel-default"><div class="panel-heading">扩展属性</div><div class="panel-body"></div></div>').appendTo(body);
			
			
			var index = 0;
			for(var i in module.conf){
				+function(){
					var row = $('<div class="row">').css('margin', '5px 10px').appendTo(pane2);
					if(index % 1 > 0){
						row.addClass('even');
					}
					index++;
			...
			pane2.find('input[type="text"]').on('input blur', function(){
				var val = $(this).val();
		...
		addNewModule: function(){
			...
			$('#btn_submit').click(function(){
				...
				if($.trim(moduleName) == '' || $.trim(moduleCaption) == ''){
					Message.warn("请检查必填项");
					return;
				}
