...
//			loadData();
	...
	getInstanceListCallback: function(cloud) {
		...
		CentralProxy.getCpuInfo(param, function(resp) {
			...
			var data = resp.data;
			cloud.instanceListDataCpu = data;
			//Cloud.createInstanceListBar(cloud);
			
			CentralProxy.getMemoryInfo(param, function(resp2) {
				if (resp2.errorMsg != null) {
					console.log(resp2.errorMsg);
					return;
				}
				var data = resp2.data;
				cloud.instanceListDataMemory = data;
				Cloud.createInstanceListBar(cloud);
			});
		});
...
//		}
		...
		/*var cpuArgs = $.map(instanceIds, function(instanceId){
			var url = 'http://paas.cloud.jstv.com/api/monitor/getCpuInfo/?platformId=openstackcj-cjy&instanceId='+ 
...
instanceId +'&beginTime='+ beginTime +'&endTime=' + endTime;
		...
		var memoryArgs = $.map(instanceIds, function(instanceId){
			var url = 'http://paas.cloud.jstv.com/api/monitor/getMemoryInfo/?platformId=openstackcj-cjy&instanceId='+ 
...
//			cloud.instanceInfo = data;
	...
	getSpaceBarOption: function(cloud){
		var fontSize = Utils.getEchartsFontSize(22, 1920);
		var labelColor = '#5793f3', monthColor = '#ffc600', weekColor = '#8c3fcb', dayColor = '#05b8a3', allColor = 
...
//							textStyle: {
//								color:"#ffffff",
//								fontFamily: "Microsoft Yahei, 微软雅黑, Helvetica Neue, Helvetica, 

//Hiragino Sans GB, Segoe UI, Tahoma, Arial, STHeiti, sans-serif",
//								fontSize: fontSize,
//								fontStyle: 'normal',
//								fontWeight: 'normal'
//							}
...
//			option.series[0].data = temp;
	...
	getInstanceListBarOption: function(cloud){
		...
		var option = {
				...
				series: [{
					...
					label: {
						normal: {
							...
							formatter: function(param) {
								return 'CPU: ' + (param.data*100).toFixed(2) + '%';
							},
							textStyle: {
								fontFamily: "Microsoft Yahei, 微软雅黑, Helvetica Neue, Helvetica, Hiragino Sans GB, Segoe UI, Tahoma, Arial, STHeiti, sans-serif",
								fontSize: 18,
					...
					label: {
						normal: {
							...
							position: 'insideLeft',
							formatter: function(param) {
								return 'RAM: ' +  (param.data*100).toFixed(2) + '%';
							},
							textStyle: {
								fontFamily: "Microsoft Yahei, 微软雅黑, Helvetica Neue, Helvetica, Hiragino Sans GB, Segoe UI, Tahoma, Arial, STHeiti, sans-serif",
								fontSize: 18,
			...
			var data = cloud.instanceListDataCpu;
			option.yAxis[0].data = [];
			option.series[0].data = [];
			option.series[1].data = [];
			for (var i = 0; i < data.length; i++) {
				...
				var cpuUsed = (data[i].cpuUsed/100).toFixed(4);
				if(cpuUsed > 1){
					cpuUsed = 1;
				}else if(cpuUsed < 0){
					cpuUsed = 0;
			...
			var dataMemory = cloud.instanceListDataMemory;
			for (var i = 0; i < dataMemory.length; i++) {
				var name = dataMemory[i].instanceName;
				if(dataMemory[i].monitorState == 'ERROR'){
					name = {
						    value: '★运行异常：' + dataMemory[i].instanceName,
						    textStyle: {
						        fontSize: fontSize,
						        color: '#fe5900'
						    }
				...
				var memoryTotal = dataMemory[i].memoryTotal;
				var memoryUsed = (dataMemory[i].memoryUsed/memoryTotal / 100).toFixed(4);
				if(memoryUsed > 1){
					memoryUsed = 1;
...
$(function() {
	...
	var title = ModuleConfigHelper.getConfigByModuleName('ns.monitor.cloud').title || '云监控';
