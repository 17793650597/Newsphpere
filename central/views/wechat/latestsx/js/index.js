var Blog = {
    ...
    loadData: function (blog, callback) {
		...
		$.ajax({
			url		: ModuleConfigHelper.getConfigByModuleName('ns.wechat.latestsx').queryUrlToken,
			type	: 'GET',
			async	: true,
//			contentType : 'application/json;charset=UTF-8',
			success	: function(response) {
			    ...
			    $.ajax({
					url		: ModuleConfigHelper.getConfigByModuleName('ns.wechat.latestsx').queryUrl,
					type	: 'POST',
					async	: true,
					contentType : 'application/json;charset=UTF-8',
...
//        blogItem.blogItemElement.find('.author-avatar-img').attr('src', blogItem.data.author.headPicPath);
        blogItem.blogItemElement.find('.author-avatar-img').attr('src', "../../../images/weixin.jpg");
        blogItem.blogItemElement.find('.author').text(blogItem.data.sourcename);
        blogItem.blogItemElement.find('.time').text(blogItem.data.pubtime);
        ...
        blogItem.blogItemElement.find('.content').text(blogItem.data.title);
//        blogItem.blogItemElement.find('.content').text(content);
        blogItem.blogItemElement.find('.share').text(blogItem.data.readnum);
//        blogItem.blogItemElement.find('.comments').text(blogItem.data.commentnum);
        blogItem.blogItemElement.find('.praise').text(blogItem.data.likenum);
        /*blogItem.blogItemElement.css('opacity', '1');*/
        blogItem.blogItemElement.css('opacity', '1').click(function(){
			DetailShow.show({
	            container: $('body'),
	            data: {
	            	title: blogItem.data.sourcename,
	            	time: blogItem.data.pubtime,
	            	content: blogItem.data.title,
	            	pics: []
	        }});
        });
