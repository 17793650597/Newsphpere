var List = {
	confs: ModuleConfigHelper.getConfigByModuleName('ns.news.city'),
	pageSize: 8,
	titlePageSize : 10,
	data: {
		rundowns: [],
	...
	createNew: function(opts) {
		...
		$('<option>').html(now.format('yyyy年MM月dd日')).appendTo('#dateSelector');
	...
	loadRundows: function() {
		...
		//TODO 实时数据
//		$.getJSON('/central/api/data/sobey/news/ns.daily.rundown', function(response){
		$.getJSON('./js/rundown.json', function(response){
			...
			$(List.options.pages).each(function(index, item){
				var li = $('<li data-target="#carousel" data-slide-to="0"></li>').appendTo('.carousel-indicators');
	...
	loadTitles: function(rundown) {
		...
		$.getJSON('/central/api/data/sobey/news/ns.daily.title?id=' + rundown.id, function(response){
//		$.getJSON('/central/views/daily/rundown/js/title.json', function(response){
	...
	appendTile: function(data, pane) {
		...
		$(data).each(function(index, item){
			...
			row.find('.logo').css('background', 'url(/central/css/' + Utils.getSkin() + '/images/columns/' + item.columnCode + '.jpg)  0% 0% / 100% 100% no-repeat rgba(0,0,0,0)');
			row.appendTo(pane);
			if(index == 0){
				row.addClass('active');
			}
    ...
    _playTitleActive: function() {
		function playRow(){
			...
			$(items[index]).addClass('active', 300);
