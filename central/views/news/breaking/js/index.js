...
//			timeNew = +timeNew.substr(11,2)+Math.ceil(Math.random())		
			...
			breakingNewsBubble.bubbleElement.find('.time').text(timeNew);
...
//	var querySite = "http://data.cloud.jstv.com:31801/service/bd/get/action=query&text=*&fieldtext=EQUAL%7B1%7D:IS_EMERGENCY&printfields=DREDATE,CATEGORY_THREE,CATEGORY_TWO,DRECONTENT&sort=date&anylanguage=true&responseformat=json&DatabaseMatch=network&maxresults=35";
	var querySite = "js/data.json";
	var breakingNewsBubble = {};
	var queryCount = 0;
	function loadData() {
        $.getJSON(querySite, function (resp) {
        	queryCount++;
        	breakingNewsBubble.queryResp = resp;
            loadDataCallBack(breakingNewsBubble);
        });
    };
	//CentralProxy.getInfos(function(response){
	//	breakingNewsBubble.title = response.directLocation;	
    ...
    function processData(breakingNewsBubble) {
        ...
        $(breakingNewsBubble.queryResp.autnresponse.responsedata["autn:hit"]).each(function (index, row) {
        	breakingNewsBubble.processedData.push({
//        		mediaSource : row["autn:content"].DOCUMENT.CATEGORY_TWO.$,
        		mediaSource : breakingNewsBubble.title + '新闻',
				time : row["autn:content"].DOCUMENT.DREDATE.$,
				titleH1 : row["autn:title"].$,
				hasPic: false,
				hasVideo: false,
				/*content: row["autn:content"].DOCUMENT.DRECONTENT ? row["autn:content"].DOCUMENT.DRECONTENT.$.substr(0,70)+'...' : '',*/
				content: row["autn:content"].DOCUMENT.DRECONTENT ? row["autn:content"].DOCUMENT.DRECONTENT.$ : '',
            });
        });
    };
...
//	Utils.setPageFontSize(1920);
////	var querySite = "http://data.cloud.jstv.com:31801/service/bd/get/action=query&text=*&fieldtext=EQUAL%7B1%7D:IS_EMERGENCY&printfields=DREDATE,CATEGORY_THREE,CATEGORY_TWO,DRECONTENT&sort=date&anylanguage=true&responseformat=json&DatabaseMatch=network&maxresults=35";
//	var conf = ModuleConfigHelper.getConfigByModuleName('ns.news.breaking');
//	var querySite = conf.queryUrl;
////	var querySite = "js/data.json";
//	var breakingNewsBubble = {};
//	var queryCount = 0;
//	function loadData() {
...
//    function processData(breakingNewsBubble) {
//    	breakingNewsBubble.processedData = [];
//        $(breakingNewsBubble.queryResp.autnresponse.responsedata["hit"]).each(function (index, row) {
//        	breakingNewsBubble.processedData.push({
//        		mediaSource : row["content"].DOCUMENT.CATEGORY_TWO.$/* + "-" + row["autn:content"].DOCUMENT.CATEGORY_THREE.$*/,
//				time : row["content"].DOCUMENT.DREDATE.$,
//				titleH1 : row["title"]?row["title"].$ : '无标题',
//				hasPic: false,
//				hasVideo: false,
//				/*content: row["autn:content"].DOCUMENT.DRECONTENT ? row["autn:content"].DOCUMENT.DRECONTENT.$.substr(0,70)+'...' : '',*/
//				content: row["content"].DOCUMENT.DRECONTENT ? row["content"].DOCUMENT.DRECONTENT.$ : '',
//            });
//        });
//    };
//    loadData();
