...
//        param.category = reportStatistics.options.key;
        $('.loading').show();
    	var title = ModuleConfigHelper.getConfigByModuleName('ns.news.analysis').title || '关键词报道：';
    	$('.title').text(title);
        //最初的温岭接口 
    	var param = {
    		"token":"",
	    ...
	    CentralProxy.getTokenPieBar("queryUrlToken", function(response){
	    	$('.loading').hide();
	    	param.token = encodeURIComponent(response.token);
	    	CentralProxy.getAnalysisData(param, function(resp){
