...
//	var querySite = "js/data.json";
	var breakingNewsBubble = {};
	var queryCount = 0;
	var title = ModuleConfigHelper.getConfigByModuleName('ns.news.consensus').title || "突发新闻";
...
//        });
	    CentralProxy.getTokenForRenmin("consensusToken", function(response){
	    	param.token = encodeURIComponent(response.token);
	    	CentralProxy.getBreakingData(param, function(resp){
	    		console.log(JSON.parse(resp))
	    		queryCount++;
	        	breakingNewsBubble.queryResp = JSON.parse(resp);
	            loadDataCallBack(breakingNewsBubble);
	    	})
	    })
    };
    ...
    function processData(breakingNewsBubble) {
        ...
        $(breakingNewsBubble.queryResp.data).each(function (index, row) {
        	breakingNewsBubble.processedData.push({
        		mediaSource : row.pubSource || '',
				time : row.pubtime || '',
				titleH1 : row.title  || '',
				hasPic: false,
				hasVideo: false,
